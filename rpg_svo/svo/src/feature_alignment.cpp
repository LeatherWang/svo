...
bool align2D(
        const cv::Mat& cur_img,    //<search_level_>
        uint8_t* ref_patch_with_border,
        uint8_t* ref_patch,
        const int n_iter, //10
        Vector2d& cur_px_estimate, //<search_level_>
        bool no_simd)
{
#ifdef __ARM_NEON__
    if(!no_simd)
    ...
    for(int iter = 0; iter<n_iter; ++iter)
    {
        int u_r = floor(u);
        int v_r = floor(v);
        // 对，`u_r >= cur_img.cols-halfpatch_size_`没问题
        if(u_r < halfpatch_size_ || v_r < halfpatch_size_ || u_r >= cur_img.cols-halfpatch_size_ || v_r >= cur_img.rows-halfpatch_size_)
            break;
        ...
        for(int y=0; y<patch_size_; ++y)
        {
            ...
            {
                float search_pixel = wTL*it[0] + wTR*it[1] + wBL*it[cur_step] + wBR*it[cur_step+1];
                float res = search_pixel - *it_ref + mean_diff; //残差，一维
                Jres[0] -= res*(*it_ref_dx); //雅可比: 1x3维
