...
Config::Config() :
    ...
    quality_min_fts(vk::getParam<int>("svo/quality_min_fts", 50)), //如果你跑代码的时候发现很容易跟丢，可以修改这几个配置参数：匹配到的最小特征点和
    quality_max_drop_fts(vk::getParam<int>("svo/quality_max_drop_fts", 40)) //容许相邻帧匹配到特征点的最大落差
#else
    trace_name("svo"),
    trace_dir("/tmp"),
    n_pyr_levels(3),
